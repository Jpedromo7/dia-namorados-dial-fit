...
import { DRAW_DATE, DRAW_DATE_LABEL, PRIZES, WINNERS_COUNT } from "@/config/campaign";
import type { CampaignEntry } from "@/types/campaign";

function shuffleEntries(entries: CampaignEntry[]) {
...
export function RaffleDraw({
  ...
  async function draw() {
    ...
    try {
      if (persistResult) {
        ...
        const selected = shuffleEntries(validated).slice(0, WINNERS_COUNT);
        if (selected.length < WINNERS_COUNT) throw new Error("Não há inscrições validadas suficientes.");
        setWinners(selected);
      }
    } catch (error) {
  ...
  return (
    <section className="campaign-frame p-5 sm:p-7">
      ...
      {winners.length ? <div className="mt-6 grid gap-3">{winners.map((winner, index) => <div key={winner.id} className="flex items-center gap-4 rounded-xl bg-[#55e814] p-5 text-[#071006]"><Trophy size={32} /><div><p className="text-xs font-black uppercase tracking-[0.14em]">{index + 1}º vencedor · {PRIZES[index]?.title}</p><p className="mt-1 text-2xl font-black">{winner.studentName}</p><p className="font-semibold">Inscrição {winner.raffleNumber} · {PRIZES[index]?.partner}</p></div></div>)}</div> : null}
    </section>
  );
}
